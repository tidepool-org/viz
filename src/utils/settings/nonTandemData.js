...
/**
 * bolusTitle
 * @param  {String} manufacturer one of: animas, carelink, insulet, medtronic, microtech, tidepool loop, diy loop, twiist loop
 *
...
export function bolusTitle(manufacturer) {
  const BOLUS_SETTINGS_LABEL_BY_MANUFACTURER = {
    ...
    'twiist loop': t('Bolus Calculator'),
  };
...
function sensitivityTitle(manufacturer) {
  const ISF_BY_MANUFACTURER = {
    ...
    'twiist loop': t('Insulin Sensitivities'),
  };
...
function targetTitle(manufacturer) {
  const BG_TARGET_BY_MANUFACTURER = {
    ...
    'twiist loop': t('Correction Range'),
  };
...
function targetRows(settings, units, manufacturer) {
  const BG_TARGET_ACCESSORS_BY_MANUFACTURER = {
    ...
    'twiist loop': { columnTwo: 'low', columnThree: 'high' },
  };
  const targetData = _.includes(['diy loop', 'tidepool loop', 'twiist loop'], manufacturer)
